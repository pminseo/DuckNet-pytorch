...
class SeparatedConv2DBlock(nn.Module):
    def __init__(self, in_channels, size=3, down=False):
        ...
        self.conv1 = nn.Conv2d(in_channels, half_channels, kernel_size=(1,size), stride=1)
        self.relu1 = nn.ReLU()
        self.bn1 = nn.BatchNorm2d(half_channels) 

        self.conv2 = nn.Conv2d(half_channels, half_channels, kernel_size=(size,1), stride=1) 
        self.relu2 = nn.ReLU() 
...
class WideScopeConv2DBlock(nn.Module):
    def __init__(self, in_channels, size=3, padding='same', down=False):
        ...
        self.conv1 = nn.Conv2d(in_channels, half_channels, kernel_size=(size, size), padding=padding, dilation=1) 
        self.relu1 = nn.ReLU() 
        self.bn1 = nn.BatchNorm2d(half_channels) 

        self.conv2 = nn.Conv2d(half_channels, half_channels, kernel_size=(size, size), padding=padding, dilation=2) 
        self.relu2 = nn.ReLU() 
        self.bn2 = nn.BatchNorm2d(half_channels) 

        self.conv3 = nn.Conv2d(half_channels, half_channels, kernel_size=(size, size), padding=padding, dilation=3) 
        self.relu3 = nn.ReLU() 
...
class ResNetConv2DBlock(nn.Module):
    def __init__(self, in_channels, dilation=1, padding='same', down=False):
        ...
        self.conv_ = nn.Conv2d(in_channels, half_channels, kernel_size=(1, 1), padding=0, dilation=dilation) 
        self.relu_ = nn.ReLU() 

        self.conv1 = nn.Conv2d(in_channels, half_channels, kernel_size=(3, 3), padding=1, dilation=dilation) 
        self.relu1 = nn.ReLU() 
        self.bn1 = nn.BatchNorm2d(half_channels) 

        self.conv2 = nn.Conv2d(half_channels, half_channels, kernel_size=(3, 3), padding=1, dilation=dilation) 
        self.relu2 = nn.ReLU() 
        self.bn2 = nn.BatchNorm2d(half_channels) 
...
class DuckV2Conv2DBlock(nn.Module):
    def __init__(self, in_channels, size=3, down=False):
        ...
        self.bn2 = nn.BatchNorm2d(in_channels)  if not down else nn.BatchNorm2d(in_channels // 2) 
    def forward(self, x):
        x = self.bn1(x)
        x1 = self.widescope(x)
        x2 = self.midscope(x)
